...
fn create_entry<'a>(group_by: &GroupBy, play_item: &'a PlayItem) -> Result<&'a dyn Group, ()> {
    let entry: &dyn Group = match group_by {
        GroupBy::Album => {
...
pub fn get_grouped_data<'a>(
    group_by: &GroupBy,
    played_items: &Vec<PlayItem>,
) -> Vec<&'a dyn Group> {
    let mut grouped_data_map: HashMap<String, &dyn Group> = HashMap::new();
